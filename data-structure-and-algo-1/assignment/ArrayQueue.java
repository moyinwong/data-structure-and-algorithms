...
public class ArrayQueue<T> {
    ...
    public void enqueue(T data) {
        ...
        if (size == backingArray.length) {
            T[] newQueue = (T[]) new Object[backingArray.length * 2];

            for (int i = front; i < backingArray.length; i++) {
                int newIndex = (i + backingArray.length - front) % backingArray.length;
                newQueue[newIndex] = backingArray[i];
            }

            int back = (front + size - 1) % backingArray.length;

            for (int j = back; j >= 0; j--) {
                int newIndex = (j + backingArray.length - front) % backingArray.length;
                newQueue[newIndex] = backingArray[j];
            }
            newQueue[size] = data;
