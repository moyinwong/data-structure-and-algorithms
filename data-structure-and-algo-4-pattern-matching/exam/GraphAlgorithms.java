...
public class GraphAlgorithms {
    ...
    public static <T> Set<Edge<T>> prims(Vertex<T> start, Graph<T> graph) {
        ...
        while (!pq.isEmpty() && visited.size() < n && mst.size() < finalMSTsize) {
            Edge<T> edge = pq.poll();
            ...
            if (!visited.contains(to)) {
                ...
                for (VertexDistance<T> v : adjList.get(to)) {
                    if (!visited.contains(v.getVertex())) {
                        pq.add(new Edge<>(to, v.getVertex(), v.getDistance()));
                    }
                }
            }
        }

        if (visited.size() < n || mst.size() != finalMSTsize) {
            return null;
        }
